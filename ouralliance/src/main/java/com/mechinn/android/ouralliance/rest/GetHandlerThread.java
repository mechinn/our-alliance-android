        ...
        uiHandler = new Handler(
                new Handler.Callback() {
                    ...
                    public boolean handleMessage(Message msg) {
                        if(GetHandlerThread.this.context instanceof Activity) {
                            ((Activity) GetHandlerThread.this.context).setProgressBarIndeterminateVisibility(msg.getData().getBoolean(PROGRESS));
                        }
                        if(null!=msg.getData().getString(STATUS)) {
                            Toast.makeText(GetHandlerThread.this.context, msg.getData().getString(STATUS), Toast.LENGTH_SHORT).show();
                        }
                        return true;
                    }
                }
